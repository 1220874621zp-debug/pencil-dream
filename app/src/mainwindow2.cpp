...
void MainWindow2::createMenus()
{
    ...
    toolsActionGroup->addAction(ui->actionOnionAlign);
    ...
    connect(toolsActionGroup, &QActionGroup::triggered, this, [&](QAction* action) {
        ...
        else if (action == ui->actionOnionAlign) mToolBox->setActiveTool(ONION_ALIGN);
...
void MainWindow2::setupKeyboardShortcuts()
{
    ...
    ui->actionOnionAlign->setShortcut(cmdKeySeq(CMD_TOOL_ONION_ALIGN));
    ...
    ui->actionOnionAlign->installEventFilter(shortcutFilter);
    ui->actionMove->installEventFilter(shortcutFilter);
...
void MainWindow2::makeConnections(Editor* pEditor, TimeLine* pTimeline)
{
    ...
    // the TVP satellite tools are always reachable from the Layer menu, even
...
{
    mCommands->importReferenceVideo();
}
...
void MainWindow2::createToolbars()
{
    ...
    // 镂空检测：一键补齐当前帧线稿的封闭镂空与细缝；连点循环切换填充方向（自动→方向一→方向二）
    {
        QAction* holeFillAction = new QAction(QIcon(":/icons/themes/playful/misc/hole-detect.svg"), tr("镂空检测"), this);
        holeFillAction->setToolTip(tr("镂空检测：自动填充当前帧的封闭镂空与细缝（每个像素取最近不透明像素的颜色）；连续点击循环切换填充方向：自动 → 方向一（竖缝左/横缝上）→ 方向二（竖缝右/横缝下），可撤销"));
        connect(holeFillAction, &QAction::triggered, mCommands, &ActionCommands::fillHolesOnCurrentFrame);
        connect(mCommands, &ActionCommands::holeFillStatusChanged, holeFillAction, &QAction::setToolTip);

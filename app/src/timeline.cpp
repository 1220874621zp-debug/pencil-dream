...
/** 摄影表"添加中割"：在指定帧处插入一张空白画格，该帧及之后的所有画格
 *  顺次右移一格（曝光长度随画格走）。整次插入为单步撤销。 */
void TimeLine::insertInbetween(int frameNumber)
{
    Layer* layer = editor()->layers()->currentLayer();
    if (layer == nullptr || !layer->isBitmapKind() || layer->locked()) { return; }
    ...
    // 插入点及之后的画格整体右移一格（停车法防相邻覆盖）
    QList<int> moves;
    for (int pos : positions)
    {
        if (pos >= frameNumber) { moves.append(pos); }
    }
    const int parkBase = layer->getMaxKeyFramePosition() + moves.count() + 1000;
    for (int i = 0; i < moves.count(); ++i)
    {
        KeyFrame* key = layer->takeKeyFrame(moves[i]);
        Q_ASSERT(key != nullptr);
        layer->addKeyFrame(parkBase + i, key);
    }
    for (int i = 0; i < moves.count(); ++i)
    {
        KeyFrame* key = layer->takeKeyFrame(parkBase + i);
        Q_ASSERT(key != nullptr);
        layer->addKeyFrame(moves[i] + 1, key);
    }

    // 中割画格本体：1x1 透明位图（与末尾块实体化空白帧同一范式）
    QImage blank(1, 1, QImage::Format_ARGB32_Premultiplied);
    blank.fill(Qt::transparent);
    BitmapImage* blankImage = new BitmapImage(QPoint(0, 0), blank);
    layer->addKeyFrame(frameNumber, blankImage);

    layer->deselectAll();
    layer->setFrameSelected(frameNumber, true);

    editor()->endLayerLayoutEdit(tr("添加中割"));

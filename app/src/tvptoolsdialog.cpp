...
InbetweenRefsDialog::InbetweenRefsDialog(Editor* editor, QWidget* parent)
    : QDialog(parent)
    , mEditor(editor)
{
    setWindowTitle(tr("对位中割参考"));
    setModal(false);
    setMinimumWidth(280);

    auto* layout = new QVBoxLayout(this);
    QLabel* hint = new QLabel(tr("把当前位图层的上一关键帧/下一关键帧生成为红/绿着色参考层，两帧内容中心自动对齐到中点，方便画中割。"));
    hint->setWordWrap(true);
    layout->addWidget(hint);

    QPushButton* createButton = new QPushButton(tr("创建 / 更新参考"));
    QPushButton* toggleButton = new QPushButton(tr("显示 / 隐藏"));
    QPushButton* resetButton = new QPushButton(tr("重置位置"));
    QPushButton* deleteButton = new QPushButton(tr("删除参考"));
    layout->addWidget(createButton);
    layout->addWidget(toggleButton);
    layout->addWidget(resetButton);
    layout->addWidget(deleteButton);

    connect(createButton, &QPushButton::clicked, this, &InbetweenRefsDialog::createOrUpdateRefs);
    connect(toggleButton, &QPushButton::clicked, this, &InbetweenRefsDialog::toggleRefsVisibility);
    connect(resetButton, &QPushButton::clicked, this, &InbetweenRefsDialog::resetRefsPosition);
    connect(deleteButton, &QPushButton::clicked, this, &InbetweenRefsDialog::deleteRefs);
}

namespace
{
    QImage tintedImage(const QImage& src, bool red)
    {
        QImage out = src.convertToFormat(QImage::Format_ARGB32);
        const qreal strength = 0.35;
        for (int y = 0; y < out.height(); ++y)
        {
            ...
            for (int x = 0; x < out.width(); ++x)
            {
                ...
                if (red)
                {
                    rgb = qRgba(static_cast<int>(r + (255 - r) * strength),
                                static_cast<int>(g * (1.0 - strength)),
                                static_cast<int>(b * (1.0 - strength)),
                                qAlpha(rgb));
                }
                else
                {
                    rgb = qRgba(static_cast<int>(r * (1.0 - strength)),
                                static_cast<int>(g + (255 - g) * strength),
                                static_cast<int>(b * (1.0 - strength)),
                                qAlpha(rgb));
                }
            }
...
void InbetweenRefsDialog::createOrUpdateRefs()
{
    Layer* current = mEditor->layers()->currentLayer();
    if (current == nullptr || current->type() != Layer::BITMAP)
    {
        QMessageBox::warning(this, tr("对位中割参考"), tr("请先选择一个位图图层。"));
        return;
    }
    LayerBitmap* sourceLayer = static_cast<LayerBitmap*>(current);
    const int frame = mEditor->currentFrame();

    const int prevPos = sourceLayer->getPreviousKeyFramePosition(frame);
    const int nextPos = sourceLayer->getNextKeyFramePosition(frame);
    BitmapImage* prevImg = (prevPos > 0 && prevPos < frame) ? sourceLayer->getBitmapImageAtFrame(prevPos) : nullptr;
    BitmapImage* nextImg = (nextPos > frame) ? sourceLayer->getBitmapImageAtFrame(nextPos) : nullptr;
    if (prevImg == nullptr && nextImg == nullptr)
    {
        QMessageBox::warning(this, tr("对位中割参考"), tr("当前帧前后都没有关键帧。"));
        return;
    }

    // both content centers align to their midpoint
    QPointF midpoint;
    bool haveBoth = (prevImg != nullptr && nextImg != nullptr);
    if (haveBoth)
    {
        midpoint = (QPointF(prevImg->bounds().center()) + QPointF(nextImg->bounds().center())) / 2.0;
    }
    else if (prevImg != nullptr)
    {
        midpoint = prevImg->bounds().center();
    }
    else
    {
        midpoint = nextImg->bounds().center();
    }

    const QString prevName = tr("中割_前帧");
    const QString nextName = tr("中割_后帧");

    if (prevImg != nullptr)
    {
        const QRect b = prevImg->bounds();
        Layer* found = mEditor->layers()->findLayerByName(prevName, Layer::BITMAP);
        LayerBitmap* refLayer = (found != nullptr) ? static_cast<LayerBitmap*>(found)
                                                   : mEditor->layers()->createBitmapLayer(prevName);
        const QImage tinted = tintedImage(prevImg->image()->copy(b), true);
        BitmapImage* refImage = new BitmapImage(b.topLeft(), tinted);
        refImage->setOpacity(0.3);
        mPrevOriginalTopLeft = b.topLeft();
        refImage->moveTopLeft(b.topLeft() + (midpoint - QPointF(b.center())).toPoint());

        mEditor->beginLayerLayoutEdit(refLayer);
        QList<KeyFrame*> olds;
        refLayer->foreachKeyFrame([&olds](KeyFrame* key) { olds.append(key); });
        for (KeyFrame* old : olds)
        {
            KeyFrame* taken = mEditor->takeLayerKeyFrame(refLayer, old->pos());
            delete taken;
        }
        refLayer->addKeyFrame(frame, refImage);
        mEditor->endLayerLayoutEdit(tr("对位参考"));
        refLayer->setVisible(true);
    }

    if (nextImg != nullptr)
    {
        const QRect b = nextImg->bounds();
        Layer* found = mEditor->layers()->findLayerByName(nextName, Layer::BITMAP);
        LayerBitmap* refLayer = (found != nullptr) ? static_cast<LayerBitmap*>(found)
                                                   : mEditor->layers()->createBitmapLayer(nextName);
        const QImage tinted = tintedImage(nextImg->image()->copy(b), false);
        BitmapImage* refImage = new BitmapImage(b.topLeft(), tinted);
        refImage->setOpacity(0.3);
        mNextOriginalTopLeft = b.topLeft();
        refImage->moveTopLeft(b.topLeft() + (midpoint - QPointF(b.center())).toPoint());
...
void InbetweenRefsDialog::toggleRefsVisibility()
{
    Layer* prev = mEditor->layers()->findLayerByName(tr("中割_前帧"), Layer::BITMAP);
    Layer* next = mEditor->layers()->findLayerByName(tr("中割_后帧"), Layer::BITMAP);
    if (prev == nullptr && next == nullptr) { return; }
    const bool show = (prev != nullptr) ? !prev->visible() : !next->visible();
    if (prev != nullptr) { prev->setVisible(show); }
    if (next != nullptr) { next->setVisible(show); }
    mEditor->getScribbleArea()->update();
    emit mEditor->updateTimeLine();
}

void InbetweenRefsDialog::resetRefsPosition()
{
    const int frame = mEditor->currentFrame();
    for (const QString& name : { tr("中割_前帧"), tr("中割_后帧") })
    {
        Layer* layer = mEditor->layers()->findLayerByName(name, Layer::BITMAP);
        if (layer == nullptr) { continue; }
        BitmapImage* img = static_cast<LayerBitmap*>(layer)->getBitmapImageAtFrame(frame);
        if (img == nullptr) { continue; }
        img->moveTopLeft(name == tr("中割_前帧") ? mPrevOriginalTopLeft : mNextOriginalTopLeft);
    }
    mEditor->getScribbleArea()->update();
}

void InbetweenRefsDialog::deleteRefs()
{
    const QStringList names = { tr("中割_前帧"), tr("中割_后帧") };

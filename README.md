...
- **设定卡片面板**：导入设定图自动提取色卡，色块悬浮于图上可拖动/框选多选/改名/双击改色/单击应用；标记线折线确认；右键新建色块；工程 main.xml 引用持久化（随工程走）
- **调色板提取**：k-means 聚类提取主色（桶众数确定性播种 + Lloyd 迭代，修复鲜艳色打散/灰阶霸榜/抗锯齿混色三类失真），生成色块图层 + 写入色彩面板色卡 + 复制色值

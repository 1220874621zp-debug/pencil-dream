...
- **参考视频层**：链接式导入（QMediaPlayer 进程内解码跟随时间轴播放，含音频输出），时间轴显示灰色胶片图标；仅 ffmpeg 无 ffprobe 环境自动兜底解析时长
...
或直接使用 `build-qt6.bat`（自动配置 vcvars + Qt 路径）。测试基线：**6591 断言 / 111 用例**全绿（含填色引擎/渲染管线/撤销事务/打组/中割算法回归）。

 ...
 *     （照明=各光源最强处），离所有光越远越暗。
 *  3. SDF 伪法线 N·L（形体明暗交界线，主阴影场）：掩膜距离变换经**连通域自适应
 *     球冠**（每白域 D=域内 dmax、按部件最大半径封顶，z=√(2Dd−d²)）当伪高度场
 *     ——每个部件鼓成自己的球冠而非共用固定半径、线稿/洞是丘间山谷；坡度
 *     (1−u)/√(2u−u²) 只依赖 u=d/D（尺度不变），交界线横切任意大小部件。
 *     按 σ 圆滑后取梯度得伪法线 N（球面 lambert：部件内部法线连续放射），
 *     照度 = Σ 强度i·max(0, N·L_i)——多光源互补照明，所有光都照不到的坡面才全暗；
 *     阴影深度 = 1−照度。

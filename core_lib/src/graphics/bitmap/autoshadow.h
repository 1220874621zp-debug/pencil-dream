...
struct AutoShadowParams
{
    ...
    int lightHeight = 150;                    // 光源高度 0..300（%）：以 max(对角线, 光到内容距离) 为基——100≈45°仰角，越大越顶光
 ...
 *
 * ── 场生成段：三路合成阴影深度，clamp 后 0..100 进映射 ──
 *  2. 圆形渐变（底场）：白区内 到光源距离−r0 按 vmax 归一化，离光越远越暗。
 *  3. SDF 伪法线 N·L（形体明暗交界线，主阴影场）：掩膜距离变换当伪高度场——每个
 *     连通区域是一座光滑小丘、线稿/洞是丘间山谷；按 σ 圆滑后取梯度得伪法线 N
 *     （高度场法线，向外），与光源向量 L 点积——阴影深度 = 1−N·L。明暗交界线
 *     横切形体（脸颊弧线、发缕各自分块），贴线阴影自动成立（山谷两侧法线相背）。
 *     这是 AE 卡渲 Relight 类插件（AI 法线打光）的解析式近似：AI 用网络估计
 *     表面朝向，我们用"角色=圆润体积"的几何先验构造朝向。
 *  4. 径向遮挡：对每像素沿射向光源采样掩膜（tent 权重，洞=0、内容外不计入），
 *     白色占比低=光被挡 → 遮挡阴影（洞/前层在背光侧投影）。
 *  5. 场 = clamp(渐变强度·渐变 + 体积强度·(1−N·L) + 遮挡) ×100。
 *
 * ── 映射段（CSP 色调设置）──

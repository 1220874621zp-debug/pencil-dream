...
struct RegionLabel
{
    ...
    QPoint anchor;            // 区域内到屏障净空最大的像素（并列取离质心近；标记点圆心）
    qint32 anchorClearance = 1; // 锚点到屏障的 Chebyshev 距离（标记点边长上限=2×净空-1，不越屏障）
    qint64 area = 0;
 ...
 * 颜色 = 目标帧区域锚点（净空最大像素，恒在区域内）经"鲁棒内容框→
 * 源帧"相对映射得预测点（内容框取线稿坐标 5%~95% 分位，细长离群
 * 笔画不撑歪映射框），吸附到预测点所在的源区域，取该区域在着色结果
 * 上的众数色——非刚性动作（四肢相对躯干各自运动）下各区域独立跟色，
 * 不再是对刚体假设敏感的单像素采样；预测点附近无源区域时退回
 * 锚点单点采样；
 * 颜色补漏：源帧每种颜色（面积≥64px）若未出现在目标标记中，只在
 * 未标记区域补画——绝不改写已标颜色/透明标记（改写会偷走已正确
 * 配对区域的颜色、并把已在场的颜色挤出目标帧，破坏"不丢色"）；
 * 邻近继承：仍未标记的封闭区域继承质心最近已标区域的颜色——始终
 * 保证每个封闭区域有一个颜色点。
